...
public class ClientHandler {
    ...
    public void blacklist() throws IOException, SQLException {
        while (true) {
            String strblacklist = in.readUTF();
            if (strblacklist.startsWith("/blacklist ")) {
                String[] tokens = strblacklist.split(" ");

                int nickId = AuthService.getIdByNick(name);
   ...
   public void changeNick() throws IOException{
        while (true) {
            String strchangeNick = in.readUTF();
            if (strchangeNick.startsWith("/changename ")){
                name = strchangeNick.split("\\s", 2)[1];
            }
        }
    }
    ...
    private void readMessages() throws IOException {
        while (true) {
            ...
            if (strFromClient.startsWith("/changename ")){
                name = strFromClient.split("\\s", 2)[1];
            }
